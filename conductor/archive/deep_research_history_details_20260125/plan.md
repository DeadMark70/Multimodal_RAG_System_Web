...
- [x] Task: Create database migration for `metadata` column
...
- [x] Task: Update Backend Schemas and Models
...
- [x] Task: Implement Persistence Logic
...
- [x] Task: Update API Services
    ...
    - [x] Update `useDeepResearch` hook to pass `conversation_id` when executing plan (ensure 'create conversation' -> 'execute plan' flow)
...
- [x] Task: Create `ResearchDetailModal` Component
    ...
    - [x] Implement Markdown Renderer for `detailed_answer` (optimize for large content)
...
- [x] Task: Error Handling & Refinement
    ...
    - [x] Optimize large Markdown rendering (consider skeleton loading)
